...
describe("normative shell and production assets", () => {
  ...
  it("pins desktop, 1180px and 900px structural transitions with tokens only", () => { const css = readFileSync(resolve(process.cwd(), "src/shell/shell.css"), "utf8"); expect(css).toContain("grid-template-columns: 280px minmax(420px, 1fr) 360px 320px"); expect(css).toContain("@media (max-width: 1180px)"); expect(css).toContain("@media (max-width: 900px)"); expect(css).toContain("border: var(--hairline)"); expect(css.match(/box-shadow:[^;]+/gu)).toEqual(["box-shadow: none", "box-shadow: none"]); expect(css.match(/border-radius:[^;]+/gu)).toEqual(["border-radius: var(--radius)", "border-radius: var(--radius)"]); expect(css).not.toContain("gradient("); const cssFiles = ["src/shell/shell.css", "src/configuration/configuration.css", "src/columns/model-pool/model-pool.css", "src/columns/graph/graph.css"].map((path) => readFileSync(resolve(process.cwd(), path), "utf8")); expect(cssFiles.join("\n")).not.toMatch(/#[0-9a-f]{3,8}\b/iu); });
  it("ships only the five designated production marks and imports the shared glyph table", () => { expect(readdirSync(resolve(process.cwd(), "src/assets/brand")).sort()).toEqual(["mark-triangle-icon-mono.svg", "mark-triangle-icon.svg", "mark-triangle-of-error-mono.svg", "mark-triangle-of-error.svg", "mark-triangle-reduction.svg"]); const graphSource = readFileSync(resolve(process.cwd(), "src/columns/graph/GraphView.tsx"), "utf8"); expect(graphSource).toContain('@arbitra/schemas/glyphs'); expect(graphSource).not.toContain("const NODE_GLYPHS"); });
});
