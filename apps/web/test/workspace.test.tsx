...
describe("configuration workspace and Model Pool", () => {
  ...
  it("round-trips a maximal configuration through edit, save, load, duplicate, validate and export", async () => {
    ...
    vi.stubGlobal("fetch", async (input: RequestInfo | URL, init?: RequestInit) => { const url = String(input); const method = init?.method ?? "GET"; const body = init?.body === undefined ? undefined : JSON.parse(String(init.body)) as Record<string, unknown>; if (url === "/configurations/validate") return json({ valid: true }); if (url === "/configurations" && method === "POST") { const record = { id: `cfg-${++id}`, name: String(body?.name), config: body?.config }; records.set(record.id, record); return json(record); } const match = url.match(/^\/configurations\/(cfg-\d+)(?:\/(duplicate|export))?$/u); const recordId = match?.[1]; if (recordId === undefined) return json({}, 404); const record = records.get(recordId); if (record === undefined) return json({}, 404); if (match?.[2] === "duplicate") { const copy = { id: `cfg-${++id}`, name: String(body?.name), config: structuredClone(record.config) }; records.set(copy.id, copy); return json(copy); } if (match?.[2] === "export") return json(record.config); return json(record); });
...
describe("live read-only workflow graph", () => {
  it("lays out four structurally different workflow JSON presets", async () => {
    const layouts = await Promise.all(Object.values(PRESET_WORKFLOWS).map(layoutWorkflow)); expect(layouts.map((layout) => layout.length)).toEqual([8, 6, 5, 5]); for (const layout of layouts) expect(layout.every(({ x, y }) => Number.isFinite(x) && Number.isFinite(y))).toBe(true);
  });
  ...
  it("renders React Flow as read-only and provides model assignment in the inspector", async () => {
    ...
    const assign = vi.fn(); vi.stubGlobal("ResizeObserver", Observer); render(<GraphView workflowJson={PRESET_WORKFLOWS["feature-simple"]} runEvents={[]} modelAliases={["reviewer-a"]} assignments={{ planner: "reviewer-a" }} onAssign={assign} />); expect(screen.getByText("workflow graph · read only")).toBeTruthy(); const plannerLabels = await screen.findAllByText("Planner"); fireEvent.click(requiredAt(plannerLabels, 0)); const selector = await screen.findByLabelText("model assignment") as HTMLSelectElement; expect(selector.value).toBe("reviewer-a"); fireEvent.change(selector, { target: { value: "reviewer-a" } }); expect(assign).toHaveBeenCalledWith("planner", "reviewer-a");
  });
});
