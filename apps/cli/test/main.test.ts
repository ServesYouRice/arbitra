...
describe("CLI commands", () => {
  it("exposes the complete public command registry with estimate implemented", () => {
    expect([...IMPLEMENTED_COMMANDS].sort()).toEqual(
      ["validate", "estimate", "run", "audit", "resume", "status", "replay", "diff", "trace", "export", "report", "requirements", "approve-requirements", "revise-requirements", "apply-requirements-revision", "respond-checkpoint", "batches", "resolve-batch", "workflow", "incremental"].sort(),
    );
