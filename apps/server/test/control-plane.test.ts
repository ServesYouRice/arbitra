...
describe("localhost control plane contracts", () => {
  it("registers every declared schema-backed route, uses localhost and has no websocket surface", async () => {
    ...
    expect(Object.keys(HTTP_ROUTE_SCHEMAS).sort()).toEqual([...ROUTE_INVENTORY, ...EVALUATION_ROUTE_INVENTORY, ...TRACE_ROUTE_INVENTORY, ...REQUIREMENTS_ROUTE_INVENTORY, ...REPLAY_ROUTE_INVENTORY].map(([method, url]) => `${method} ${url}`).sort());
