...
export async function planWithContext(input: PlannerInput, port: PlannerCompositionPort): Promise<PlanIR> {
  ...
  for (const id of ids) {
    ...
    if (await port.fits(briefRequest(proposed))) { current = proposed; continue; }
  ...
  const outlineRequest: PlannerStage = { activityId: "planner/outline",
  ...
  if (outline.mode !== "audit" || JSON.stringify(outline.premiseReport) !== JSON.stringify(input.premiseReport)) throw new Error("MODEL_PLAN_PROVENANCE_MISMATCH");
  const diagnostics = validateTraceability({ ...outline, tasks: outline.tasks.map((task) => ({ ...task, context: [] })) }, ids);
  ...
  const finalDiagnostics = validateTraceability(plan, ids);
