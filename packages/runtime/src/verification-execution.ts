...
export class VerificationExecutor {
  ...
  execute(snapshot: RepositorySnapshot, policy: VerificationExecution, sourcePaths: readonly string[], signal: AbortSignal): Promise<{ records: readonly VerificationExecutionRecord[]; deferredCheckIds: readonly string[] }> {
    const work = this.#pending.then(() => this.executeSerial(snapshot, policy, sourcePaths, signal));
    this.#pending = work.catch(() => undefined);
    return work;
  }

  private async executeSerial(snapshot: RepositorySnapshot, policy: VerificationExecution, sourcePaths: readonly string[], signal: AbortSignal) {
    ...
    for (const check of execution.checks.filter(({ sourcePaths: selected }) => selected.some((path) => paths.has(path)))) {
      ...
      const id = createHash("sha256").update(JSON.stringify({ digest, execution, check })).digest("hex");
