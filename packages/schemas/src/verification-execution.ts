...
export const verificationExecutionSchema = z.strictObject({
  ...
  image: z.string().regex(/^[a-zA-Z0-9][a-zA-Z0-9._/:_-]*@sha256:[a-f0-9]{64}$/u, "Expected a local Docker image pinned by digest (name@sha256:<64 lowercase hex>). Tags alone are rejected; arbitra never pulls or builds images."),
