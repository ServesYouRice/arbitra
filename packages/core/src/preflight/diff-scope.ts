...
function validateRevision(value: string): void { if (value.trim() === "" || value.startsWith("-") || /[\s~^:?*]/u.test(value) || value.includes("[") || value.includes("\\")) throw new Error(`INVALID_DIFF_REVISION:${value}`); }
