...
function manifestFromPlan(fixture: AuditFixture, plan: PlannerPlan, canonical: CanonicalIssueSet): unknown {
  ...
  if (task === undefined) throw new Error("HANDOFF_PLAN_TASK_MISSING");
...
function withIsolatedInputs<T>(repoDir: string, implementationDir: string, operation: (paths: { root: string; repository: string; implementation: string }) => T): T {
  ...
  const repositorySource = sources[0]; const implementationSource = sources[1];
  if (repositorySource === undefined || implementationSource === undefined) throw new Error("FRESH_EXECUTOR_SOURCE_MISSING");
  ...
  try { cpSync(repositorySource, repository, { recursive: true }); cpSync(implementationSource, implementation, { recursive: true }); if (existsSync(join(root, ".runs"))) throw new Error("FRESH_EXECUTOR_RUN_CONTEXT_LEAK"); return operation({ root, repository, implementation }); }
