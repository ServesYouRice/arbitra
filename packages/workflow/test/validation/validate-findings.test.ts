...
describe("deterministic finding validation", () => {
  ...
  it.each([
    ...
    ["path_not_in_snapshot", finding({ locations: [{ id: "loc-1", path: "constructor", startLine: 1, endLine: 2 }] })],
    ["invalid_line_range", finding({ locations: [{ id: "loc-1", path: "src/auth.ts", startLine: NaN, endLine: NaN }] })],
    ["invalid_line_range", finding({ locations: [{ id: "loc-1", path: "src/auth.ts", startLine: 1.5, endLine: 2 }] })],
