...
export interface VerificationTools {
  ...
  runAllowlistedSafeTest(item: VerificationItem, policy: { readonly executionPolicy: "allowlisted" | "derived_repository_script"; readonly command: string }): Promise<VerificationAttempt>;
...
export async function runDeterministicLadder(item: VerificationItem, tools: VerificationTools): Promise<{ readonly resolved: VerificationAttempt | null; readonly attempts: readonly VerificationAttempt[] }> {
  ...
  const calls: Array<() => Promise<VerificationAttempt>> = [
    ...
    ...(allowlistedTest === undefined ? [] : [() => tools.runAllowlistedSafeTest(item, { executionPolicy: item.testExecutionPolicy ?? "derived_repository_script", command: allowlistedTest })]),
